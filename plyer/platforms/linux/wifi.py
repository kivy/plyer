		...
		for dev in devs:
			if dev.get_device_type() == NM.DeviceType.WIFI:
				for ap in dev.get_access_points():
					...
					data['ssid'] = str(ssid_to_utf8(ap))
					data['bssid'] = str(ap.get_bssid())
					data['frequency'] = str(frequency)
					data['channel'] = str(NM.utils_wifi_freq_to_channel(frequency))
					data['mode'] = str(mode_to_string(ap.get_mode()))
					data['flags'] = str(flags_to_string(flags))
					data['wpa_flags'] = str(security_flags_to_string(wpa_flags))
					data['rsn_flags'] = str(security_flags_to_string(rsn_flags))
					data['security'] = str(flags_to_security(flags, wpa_flags, rsn_flags))
